...
-- =========================
ngx.log(ngx.INFO, "[DEBUG ENV] S3_BUCKET=", os.getenv("S3_BUCKET"))
ngx.log(ngx.INFO, "[DEBUG ENV] AWS_REGION=", os.getenv("AWS_REGION"))
ngx.log(ngx.INFO, "[DEBUG ENV] AWS_ACCESS_KEY_ID=", os.getenv("AWS_ACCESS_KEY_ID"))
ngx.log(ngx.INFO, "[DEBUG ENV] AWS_SECRET_ACCESS_KEY=****")
ngx.log(ngx.INFO, "[DEBUG ENV] SERVICE_NAME=", os.getenv("SERVICE_NAME"))
...
local function upload_s3_async(premature, data)
    ...
    ngx.log(ngx.INFO, "[DEBUG S3] Uploading key=", key, " size=", #data)
    ...
    if not res then
        ngx.log(ngx.INFO, "upload_s3_async failed: ", err)
        return
    end
    
    if res.status < 200 or res.status >= 300 then
        ngx.log(ngx.INFO, "S3 upload returned non-2xx status: ", res.status)
        ngx.log(ngx.INFO, "Response body: ", res.body or "nil")
        return
    end
    
    ngx.log(ngx.NOTICE, "S3 upload successful: s3://"..bucket.."/"..key)
end
...
local function send_clickhouse(premature, payload)
    ...
    if not res then
        ngx.log(ngx.INFO, "clickhouse insert failed: ", err)
    end
    httpc:set_keepalive()
end
...
if not ok then
    ngx.log(ngx.INFO, "failed to create clickhouse timer: ", err)
end
